  ...
  return (
    ...
    >
      <div className="flex items-center border-b pb-2 mb-2">
        ...
        <Tag color="yellow" className="uppercase">{data.status}</Tag>
      </div>
      ...
      <div className="flex items-start my-3">
        <div className="flex-1">
          ...
          <p className="font-semibold">
            {data.user?.fullName} <br /> {moment(data.createdAt).format('DD-MM-YYYY HH:mm A')}
          </p>
        </div>
        <div>
          <span className="text-sm text-gray-500">Start Date</span>
          <p className="font-semibold">{moment(data.startDate).format('DD-MM-YYYY')}</p>
        </div>
      </div>
