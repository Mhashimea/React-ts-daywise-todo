...
export default function DatePicker({ emitDate }: any) {
  ...
  const generateDate = (month: number, year: number) => {
    ...
    setCurrentMonth(moment(datesArry[0]).format("MMMM"));
    setCurrentYear(moment(datesArry[0]).format("YYYY"));
    setSelectedDate(moment(datesArry[0]).format("DD"));
    setDates(datesArry);
  };
  ...
  const onChangeMonth = (type: string) => {
    const month = moment().month(currentMonth).format("M");
    if (type === "NEXT") {
